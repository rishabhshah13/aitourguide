...
def test_invalid_model():
    ...
    response = client.post("/get_response", json={"question": "What is the weather today?", "model": "InvalidModel"})
...
def test_valid_query_payload():
    start_time = time.time()
    response = client.post("/get_response", json={"question": "What is the weather today?", "model": "GPT4o"})
    latency = time.time() - start_time
...
def test_invalid_query_payload():
    start_time = time.time()
    response = client.post("/get_response", json={"question": "What is the weather today?"})
    latency = time.time() - start_time
